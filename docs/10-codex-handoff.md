...
| `profiles\mS4AB4img2oz.yaml` | merge/DNS：ipv6 false、国内 DNS 直连、国外 DoH 经线路冗余、proxy-server-nameserver | 是 |
...
- 健康检查 300s → 120s。
- 仓库文档 `docs/01`–`10`、部署脚本、只读出口探针已在；节点筛选兼容入口不再改运行时配置。后续未明确要求时不要 commit/push。

**最近一次分流实测（2026-09-11，rule 模式）：**

- ChatGPT `cdn-cgi/trace` → 日本住宅（符合 D）。
- 一般 `api.ipify.org` → 自建 VPS 出口；若自建 204 失败，仍允许按 `线路冗余` 顺序切到机场或 CF，**不是**住宅。
- 只读探针 `scripts/verify-egress.ps1`：ChatGPT `loc=JP`、x.com、twitter.com、MATCH 有出口、百度均通过；未切换 `global`，未写运行时配置。
...
1. **DNS detour（手册 §4.3）**  
   已完成：国外 DoH 经 `线路冗余`，国内 DNS 直连，`ipv6: false`，并配置 `proxy-server-nameserver`；改动已写入 `mS4AB4img2oz.yaml`。

2. **Hysteria2（可选，弱网）**  
   已测 VPS UDP，当前探测超时，因此不启用 Hy2；继续保持 TCP Reality，仍不要把住宅塞进线路冗余组。

3. **VPS 加固（手册 Phase 1 剩余）**  
   非 root 用户密钥登录、firewalld 仅放 22/443 已完成；密码登录和 root 远程登录仍保持原状，待用户确认后再关闭。已有 BBR、SELinux Permissive。
...
- `docs/02`：已同步 AI/X/Google/Netflix → `MIYA-STATIC`，TG/Discord/YouTube/MATCH → `线路冗余`（自建 → 机场 → CF）、interval 120、`mode=global` 回写 502、命名管道不可靠和覆盖层规则。
- `README.md`、`docs/03`、`docs/06`：已同步 C+D 架构与线路顺序。
- `docs/04`：已补 GitHub 项目复核、身份探针和住宅串联前置条件。
- `scripts/filter-best-node.ps1`：已改为只读兼容入口；实际验收使用 `scripts/verify-egress.ps1`。
- 提交前自查无真实 IP/密码/订阅。
...
- [x] ChatGPT / X 出口 = 日本住宅路径，且当前探针稳定
- [x] 国内站不绕代理
- [x] 一般流量当前命中自建；自建挂了允许自动到机场或 CF
- [x] MIYA-STATIC 仅含住宅节点，住宅失败不静默变成机房 IP
