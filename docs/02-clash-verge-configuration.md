...
**机场层的内部冗余**：上面只解决了「线路类型」的冗余（自建 / 机场 / CF 三种故障域），没有解决单个机场自身抽风的问题。用 `proxy-providers` 把多个订阅聚合成一个优选池、再替换掉这里的单一「机场」层——见 `docs/12`。

⚠️ 若把聚合池放到该组首位（速度优先），该组出口会每 `interval` 变化一次；而本仓库 DNS 的 DoH 走 `#线路冗余`，顺序变化会同步影响 DNS 稳定性。默认顺序保持「自建优先」。
