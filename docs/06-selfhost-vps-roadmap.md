...
线路冗余组固定为“自建 VPS → 机场 → CF”，三者互相替代；MIYA-STATIC 是独立的住宅出口组，只服务 AI/X/Google/Netflix，住宅失败不切线路冗余。
...
| DediRock | $9.88/年起 | 极低价年付，新商家无 SLA，仅探针/备份/练手；优先 NY/Buffalo 避开 LA（深度调研见 docs/07） |

> 低价年付机的通用原则：默认当"可丢弃资源"，不放唯一数据、不承载唯一线路；买前盯库存可用第三方监控站 API 轮询（抓取技巧见 docs/07 第三节）。
...
2. 重组线路结构：线路冗余固定为自建 VPS → 机场 → CF，住宅保持独立 Fail Closed 角色
3. 端到端验证出口与吞吐
4. 同步本仓库
...
| 方案 | 月成本 | 稳定性 | 可控性 |
|---|---|---|---|
| 机场（线路冗余第二层） | 已付 | ⚠️ 作为自建 VPS 失败后的备份 | ❌ 零 |
