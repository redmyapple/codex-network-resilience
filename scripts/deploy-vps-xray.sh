#!/usr/bin/env bash
# ============================================================
# deploy-vps-xray.sh — 全新 VPS 一键部署
# Xray VLESS + REALITY + Vision (TCP 443)
#
# 支持: Ubuntu / Debian / AlmaLinux / Rocky / RHEL / CentOS Stream
#
# 用法 (root):
...
# AlmaLinux/RHEL 默认 SELinux=Enforcing, nobody + bind 443 会被拦
if command -v getenforce >/dev/null 2>&1; then
  if [[ "$(getenforce)" == "Enforcing" ]]; then
    echo "    SELinux Enforcing -> Permissive (否则 Xray 无法监听 443)"
    setenforce 0
...
if command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld; then
  ...
  echo "    firewalld 已放行 TCP $PORT / 22"
fi
# 兜底: 部分镜像只用 nft/iptables 且 firewalld 未运行
...
echo "  - 可选加固: Hysteria2 (UDP) 入口冗余、WARP 出口 (AI 解锁),见仓库 docs/06"
